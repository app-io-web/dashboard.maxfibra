// src/components/internal-services/BannersEmpresariaisSection.tsx
...
const MODEL_IMAGE_PATH = "/imagens/models/Modelo_imagens_section.png";
...
export function BannersEmpresariaisSection() {
  ...
  async function fetchBanners(showReloadSpinner = false) {
    try {
      ...
      const res = await api.get("/site/banners-empresariais");
      ...
      // Ordenar por CreatedAt desc (igual vibe anterior)
      const sorted = [...data].sort((a, b) => {
        const aCreated = a.fields.CreatedAt || "";
        const bCreated = b.fields.CreatedAt || "";
        return bCreated.localeCompare(aCreated);
      });

      setBanners(sorted);
    } catch (err: any) {
      console.error("[BANNERS_EMPRESARIAIS][LIST] Erro:", err);
      setError("Erro ao carregar os banners empresariais.");
    } finally {
      setLoading(false);
      setReloading(false);
    }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (form.id) {
        // PATCH /site/banners-empresariais/:id
        await api.patch(`/site/banners-empresariais/${form.id}`, payload);
        setSuccessMessage("Banner empresarial atualizado com sucesso.");
      } else {
        // POST /site/banners-empresariais
        await api.post("/site/banners-empresariais", payload);
        setSuccessMessage("Banner empresarial criado com sucesso.");
      }
      ...
      console.error("[BANNERS_EMPRESARIAIS][SAVE] Erro:", err?.response || err);
      setError("Erro ao salvar o banner empresarial.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete(id: string | number) {
    ...
    const confirmed = window.confirm(
      "Tem certeza que deseja excluir este banner empresarial?"
    );
    if (!confirmed) return;
    ...
    try {
      // DELETE /site/banners-empresariais/:id
      await api.delete(`/site/banners-empresariais/${idStr}`);
      setSuccessMessage("Banner empresarial deletado com sucesso.");
      await fetchBanners(true);
      ...
      console.error("[BANNERS_EMPRESARIAIS][DELETE] Erro:", err?.response || err);
      setError("Erro ao deletar o banner empresarial.");
    } finally {
      setDeletingId(null);
    }
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-slate-800">
              Banners - Site Empresarial
            </h1>
            <p className="text-xs text-slate-500">
              Gerencie os banners da página empresarial (desktop, 2K, 4K e mobile).
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-slate-800">
              {isEditing ? "Editar banner empresarial" : "Novo banner empresarial"}
            </h2>
            {isEditing && (
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="Ex: Banner Empresarial Principal"
              />
            </div>

            <div className="flex items-center gap-2 pt-1">
              <input
                id="ativo_empresarial"
                type="checkbox"
                checked={form.ativo}
              ...
              />
              <label
                htmlFor="ativo_empresarial"
                className="text-xs font-medium text-slate-700"
              >
                Banner ativo no site empresarial
              </label>
            </div>
        ...
        <div className="lg:col-span-2 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-slate-800">
              Banners empresariais cadastrados
            </h2>
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            {loading ? (
              <div className="px-4 py-6 text-xs text-slate-500">
                Carregando banners empresariais...
              </div>
            ) : banners.length === 0 ? (
              <div className="px-4 py-6 text-xs text-slate-500">
                Nenhum banner empresarial cadastrado ainda.
              </div>
            ) : (
